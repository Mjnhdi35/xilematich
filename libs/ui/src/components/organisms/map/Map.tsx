...
export const Map = ({ height = 'calc(100vh-6rem)', ...props }: MapProps) => {
  return (
    <MapGl
      ...
      mapStyle="mapbox://styles/mdjsemi/cm7bjsbnq001x01s28sac6fg3"
...
export const StyleMap = () => {
  ...
  current?.on('style.load', () => {
    current?.getMap().setFog({
      ...
      'horizon-blend': 0.06, // Atmosphere thickness (default 0.2 at low zooms)
      'space-color': 'rgb(150, 150, 150)', // Background color
      'star-intensity': 0.65, // Background star brightness (default 0.35 at low zoooms )
    })
  })
  return null
}
